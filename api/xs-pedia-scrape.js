...
// ROOT ROUTE: kompatibel dengan loader project yang menampilkan 1 file = 1 endpoint.
// Frontend cukup memanggil /api/xs-pedia/xs-pedia-scrape dengan parameter yang sesuai.
// Pemilihan endpoint dilakukan otomatis dari parameter yang dikirim.
router.get('/', async (req, res) => {
  const q = req.query || {};
  const has = (key) => typeof q[key] === 'string' && q[key].trim() !== '';

  if (has('phone') && !has('otp') && !has('otp_token') && !has('amount') && !has('token') && !has('refresh_token')) {
    return proxyGet(res, '/auth/otp', { phone: q.phone.trim() });
  }

  if (has('otp') && has('otp_token')) {
    return proxyGet(res, '/auth/verify', {
      otp: q.otp.trim(),
      otp_token: q.otp_token.trim()
    });
  }

  if (has('amount') && has('static_qr')) {
    const amount = q.amount.trim();
    if (!/^\d+(?:\.\d+)?$/.test(amount) || Number(amount) <= 0) {
      return res.status(400).json({ success: false, message: 'Parameter "amount" harus berupa angka lebih dari 0.' });
    }
    return proxyGet(res, '/api/qris/create', {
      amount,
      static_qr: q.static_qr.trim()
    });
  }

  if (has('refresh_token')) {
    return proxyGet(res, '/auth/refresh/token', {
      refresh_token: q.refresh_token.trim()
    });
  }

  if (has('token')) {
    const type = String(q.type || q.endpoint || 'history').trim().toLowerCase();
    const target = type === 'validate' ? '/auth/check/token' : '/api/history';
    return proxyGet(res, target, { token: q.token.trim() });
  }

  return res.status(400).json({
    success: false,
    message: 'Parameter tidak sesuai. Gunakan phone, otp+otp_token, amount+static_qr, token(+type=validate), atau refresh_token.'
  });
});
